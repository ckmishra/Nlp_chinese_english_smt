...
optparser.add_option("-b", "--bigramcounts", dest='counts2w', default=os.path.join('../data', 'count_2w.txt'),
                     help="bigram counts")
optparser.add_option("-i", "--inputfile", dest="input", default=os.path.join('../data', 'input'),
...
PwJoint = PdistJoint(opts.counts2w, N=Pw.N)
old = sys.stdout
        ...
        while not pq.empty():
            ...
            for k in range(endIndex + 1, min(endIndex + 1 + Pw.maxlen, len(input)) + 1):
                ...
                if Pw(newWord) is None:
                    ...
                    if joint_prob is None or joint_prob == 0.0:
                        joint_prob = math.log((1 + 0.000001) / ((0.000001) * Pw.N + math.exp(Pw(prev_word))))
